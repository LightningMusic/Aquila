"""ASUS BIOS provider for Project Aquila.

Backend reality (researched, not assumed):

Unlike Gigabyte and MSI, ASUS **does** publish a real BIOS-settings
management interface for its commercial fleet -- the "ASUS BIOS Config Tool"
(ACT). Per ASUS's own support documentation, ACT communicates over WMI, is
scriptable via command line (including from Windows PE), and can manage
three setting groups (General, Security, Password), including JSON
import/export. It requires one of two prerequisite driver packages to be
installed first, depending on the WMI protocol version the platform
exposes:

* WMI < 3.0: "ASUS Business Utility" v3.5.15.0+
* WMI 3.0+: "ASUS Business Utility" v3.5.19.0+
* WMI 4.0+: "ASUS Business System Control Interface" (BSCI) v0.4.3.0+

ACT is limited to ASUS **commercial** notebooks, desktops, and All-in-One
PCs from 2022 or later with Intel 12th-generation CPUs or newer. It is not
available on ASUS consumer/gaming boards (ROG, TUF, Prime), which -- like
Gigabyte and MSI -- have no documented scriptable settings interface.

The BSCI driver identifies itself with the ACPI hardware ID
``ACPI\\CSCI2022``, which is independently verifiable and used here to
*detect* whether the commercial settings backend is actually present on a
given system (via ``Win32_PnPEntity``), the same way the HP provider
detects the ``HP_BIOSSetting`` WMI class before trusting it.

What this provider deliberately does **not** do: guess at ACT's/BSCI's
specific WMI class and method names. ASUS has not published those class
names in any documentation this research could locate (unlike Dell, Lenovo,
and HP, whose WMI class contracts are extensively documented in vendor and
community deployment guides). Writing firmware settings against a guessed,
unverified method signature is a real safety risk -- see GP-001 (Safety
Before Automation) -- so individual BIOS setting read/write honestly falls
back to ``DefaultProvider``'s unsupported behavior even when the BSCI
driver is detected, with the detection result surfaced in diagnostics so a
future revision (once the class contract is verified against real hardware
or ASUS's own SDK documentation) knows exactly where to plug it in.

What this provider does support with real, tested logic (identical
approach to the Gigabyte/MSI providers, all through vendor-neutral
interfaces):

1. Firmware identity via ``Win32_BIOS`` / ``Win32_ComputerSystem`` /
   ``Win32_ComputerSystemProduct`` (Windows) or sysfs (Linux).
2. Boot-order management via standard UEFI firmware variables
   (``efibootmgr`` / ``bcdedit /enum firmware``).
3. TPM state via ``Win32_Tpm`` and battery presence via ``Win32_Battery``.
4. Detection of whether the commercial ACT/BSCI backend is present, for
...
_BSCI_HARDWARE_ID = "ACPI\\CSCI2022"

logger = logging.getLogger(__name__)


class ASUSProvider(DefaultProvider):
    """Production ASUS firmware provider.

    Supports ASUS desktop motherboards and laptops (consumer and
    commercial) for firmware identity, boot-order management, TPM/battery
    presence reporting, and diagnostics. Individual BIOS setting management
    is not available because this provider only trusts a verified WMI
    contract, and ASUS's commercial ACT/BSCI settings interface has not
    published one publicly (see module docstring); all setting-level
    ...
    def __init__(self, *args: Any, **kwargs: Any) -> None:
        ...
        self._asus_logger = logging.getLogger(f"{__name__}.{self.__class__.__name__}")
        self._execution_lock = threading.RLock()
        self._wmi_handle: Any = None
        self._efibootmgr_path: Optional[Path] = None
        self._bsci_present: Optional[bool] = None
    ...
    def vendor(self) -> BIOSVendor:
        """Return the ASUS vendor identifier."""
        return BIOSVendor.ASUS
    ...
    def provider_version(self) -> str:
        """Return the ASUS provider implementation version."""
    ...
    def supports_linux(self) -> bool:
        ...
        management through standard UEFI mechanisms; ACT/BSCI is
        Windows-only.
        """
        return True

    def detect(self) -> bool:
        """Return whether this system is confidently identified as ASUS."""
        return self.detection_confidence() >= 0.7

    def detection_confidence(self) -> float:
        """Calculate a confidence score from independent ASUS indicators."""
        ...
        if "asustek" in normalized_vendor or normalized_vendor == "asus":
            score += 0.55
        elif "asustek" in normalized_board or normalized_board == "asus":
            score += 0.5

        if re.search(
            r"\b(rog|tuf|prime|expertbook|expertcenter|zenbook|vivobook|"
            r"proart|strix|crosshair|maximus|zenith)\b",
            combined,
        ):
            score += 0.2

        if self._bsci_driver_present():
            score += 0.1
    ...
    def _collect_firmware_information(self) -> FirmwareInformation:
        ...
        interfaces, not ASUS-specific ones. Every other platform falls back
        ...
        manufacturer = self._first_property(system_rows, "Manufacturer") or "ASUSTeK COMPUTER INC."
        model = (
    ...
    def _detect_bios_mode() -> BIOSMode:
        ...
        if platform.system() == "Windows":
            firmware_type = ASUSProvider._windows_firmware_type()
    ...
    # ASUS's commercial ACT/BSCI backend is real (see module docstring) but
    # its WMI class and method contract is not published anywhere this
    # research could verify. ``_apply_setting`` intentionally returns
    # ``False`` (the ``DefaultProvider`` default) rather than guessing at
    # an unverified method signature and risking a silent misconfiguration
    # on real firmware -- a direct application of GP-001 (Safety Before
    # Automation). ``_bsci_driver_present()`` below still detects and
    # reports whether the backend driver is installed, so a future
    # revision that verifies the class contract (against real ASUS
    # commercial hardware, or ASUS's own SDK documentation once available)
    # has a ready detection signal and knows exactly where to add
    # ``_register_setting`` calls, following the Lenovo/HP pattern.

    def _bsci_driver_present(self) -> bool:
        """Return whether the ASUS BSCI/ACT driver is installed.

        Detected via the driver's ACPI hardware ID (``ACPI\\CSCI2022``,
        confirmed from ASUS's own driver-distribution metadata) rather than
        a guessed WMI settings class, so this detection is accurate even
        though this provider does not act on it for setting management.
        """
        if self._bsci_present is not None:
            return self._bsci_present

        if platform.system() != "Windows":
            self._bsci_present = False
            return False

        rows = self._query_wmi_safe(
            r"root\cimv2",
            "SELECT DeviceID FROM Win32_PnPEntity WHERE "
            f"DeviceID LIKE '{_BSCI_HARDWARE_ID}%'",
        )
        self._bsci_present = bool(rows)
        return self._bsci_present
        ...
        for index, number in enumerate(ordered_numbers):
            ...
            devices.append(
                BootDevice(
                    ...
                    device_type=ASUSProvider._boot_device_type(
    ...
    def firmware_update_supported(self) -> bool:
        """Return whether the ASUS BIOS Config Tool is installed.

        This only reports whether the tool is present; ACT's exact silent
        update-check invocation is not documented in accessible sources, so
        this provider does not attempt to invoke it.
        """
        return self._locate_act() is not None
    ...
    def check_for_firmware_updates(self) -> bool:
        ...
        Aquila does not itself download or install vendor firmware; this
        reports tool presence only.
        """
        self._record_operation("check_for_firmware_updates")
        return self.firmware_update_supported()

    def _locate_act(self) -> Optional[Path]:
        """Locate the ASUS BIOS Config Tool, when installed, informationally."""
        candidates = (
            Path(r"C:\Program Files (x86)\ASUS\ASUS BIOS Config Tool"),
            Path(r"C:\Program Files\ASUS\ASUS BIOS Config Tool"),
        )
    ...
    def tpm_state(self) -> TPMState:
        ...
        generic Windows interface, not an ASUS-specific one.
        """
        if platform.system() != "Windows":
            return TPMState()
    ...
    def battery_present(self) -> bool:
        ...
        Windows interface, not an ASUS-specific one.
        """
        if platform.system() != "Windows":
    ...
    # ACT's "General" setting group can, per ASUS's documentation, include
    # battery-related items on some commercial notebooks, but without a
    # verified class/method contract this provider cannot safely expose
    # numeric charge-limit management, so ``battery_charge_limit_supported()``
    # correctly falls back to ``DefaultProvider``'s honest ``False``.

    # -------------------------------------------------------------------
    # ASUS-specific extensions
    # -------------------------------------------------------------------
    ...
    def commercial_management_available(self) -> bool:
        """Return whether the ASUS commercial ACT/BSCI backend is detected.

        ``True`` indicates the platform is eligible for and has the driver
        installed for ASUS's WMI-based commercial BIOS Config Tool; it does
        not mean this provider can act on it (see module docstring for why
        setting-level management remains unimplemented).
        """
        return self._bsci_driver_present()
        ...
        base_summary.update(
            {
                ...
                "commercial_management_available": self.commercial_management_available(),
            }
        )
        return base_summary
    ...
    def report(self) -> Dict[str, Any]:
        ...
        base_report["asus"] = {
            ...
            "bios_settings_management_reason": (
                "ASUS's commercial ACT/BSCI backend was detected but its "
                "WMI class contract is not published in any accessible "
                "documentation; this provider will not guess at an "
                "unverified method signature for firmware writes."
                if self.commercial_management_available()
                else "ASUS publishes no documented settings interface for "
                "this system (consumer boards, or the commercial ACT/BSCI "
    ...
    def diagnostics(self) -> Dict[str, Any]:
        ...
        base_diagnostics.update(
            {
                "act_tool_found": self._locate_act() is not None,
                "bsci_driver_present": self._bsci_driver_present(),
        ...
        if self.commercial_management_available():
            issues.append(
                ValidationError(
                    ...
                    message=(
                        "ASUS commercial BIOS management (ACT/BSCI) is "
                        "present on this system, but its WMI class "
                        "contract has not been verified, so individual "
                        "BIOS setting management is not available through "
                        "this provider."
                    ),
                    code="bios_settings_backend_unverified",
                    severity="info",
                )
            )
        else:
            issues.append(
                ValidationError(
                    field="bios_settings_backend",
                    message=(
                        "ASUS publishes no documented interface for "
                        "individual BIOS setting management on this "
                        "system; only firmware identity, boot order, TPM "
                        "state, and battery presence are available."
                    ),
    ...
    def _query_wmi_safe(self, namespace: str, query: str) -> List[Any]:
        ...
        try:
            ...
            self._asus_logger.debug("WMI query was unavailable: %s", exc)
            return []
    ...
    def _first_property(rows: List[Any], name: str) -> str:
        for row in rows:
            value = ASUSProvider._safe_property_value(row, name)
            if value:
                return str(value)
