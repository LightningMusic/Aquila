"""Gigabyte BIOS provider for Project Aquila.

Backend reality (researched, not assumed):

Like MSI, **Gigabyte does not publish a documented WMI, PowerShell, or
command-line interface for reading or writing individual BIOS/UEFI
settings**. Gigabyte's own tooling (APP Center / Control Center / @BIOS) is
GUI-only and targeted at end users, not IT deployment; @BIOS itself is a
Windows application for flashing firmware images, not a settings-management
API, and exposes no scriptable interface for individual setting values.
Unlike ASUS, Gigabyte has no published commercial-fleet management utility
comparable to ASUS's Business System Control Interface or Dell/Lenovo/HP's
WMI settings classes.

Consequently this provider follows the same honesty policy as
``MSIProvider``: real, tested implementations of everything genuinely
available through vendor-neutral mechanisms (firmware identity, boot order,
TPM state, battery presence), and an honest "unsupported" report -- via
``DefaultProvider``'s existing fallback behavior -- for individual BIOS
setting management, virtualization/IOMMU/SR-IOV toggling, BIOS password
management, wake-source configuration, and battery charge-limit management,
rather than fabricating a nonexistent backend.

If Gigabyte later publishes a documented settings interface, ``_apply_setting``
is the extension point -- see the Lenovo/HP providers for the pattern to
follow.
"""
...
class GigabyteProvider(DefaultProvider):
    """Production Gigabyte firmware provider.

    Supports Gigabyte desktop motherboards and laptops (AORUS included) for
    firmware identity, boot-order management, TPM/battery presence
    reporting, and diagnostics. Individual BIOS setting management is not
    available because Gigabyte publishes no scriptable interface for it
    (researched, not assumed); all setting-level methods honestly fall back
    to ``DefaultProvider``'s unsupported behavior.
    """

    def __init__(self, *args: Any, **kwargs: Any) -> None:
        """Initialize provider state without opening privileged resources."""
        super().__init__(*args, **kwargs)
        self._gigabyte_logger = logging.getLogger(
    ...
    def vendor(self) -> BIOSVendor:
        """Return the Gigabyte vendor identifier."""
        return BIOSVendor.GIGABYTE
    ...
    def provider_version(self) -> str:
        """Return the Gigabyte provider implementation version."""
    ...
    def detect(self) -> bool:
        """Return whether this system is confidently identified as Gigabyte."""
        return self.detection_confidence() >= 0.7

    def detection_confidence(self) -> float:
        """Calculate a confidence score from independent Gigabyte indicators."""
        ...
        if "gigabyte" in normalized_vendor:
            score += 0.55
        elif "gigabyte" in normalized_board:
            score += 0.5

        if re.search(
            r"\b(aorus|gaming\s?x|elite|ultra\s?durable|vision|designare|"
            r"waterforce|xtreme|master)\b",
            combined,
        ):
            score += 0.2
    ...
    def _collect_firmware_information(self) -> FirmwareInformation:
        ...
        interfaces, not a Gigabyte-specific one. Every other platform falls
        ...
        manufacturer = self._first_property(system_rows, "Manufacturer") or "Gigabyte"
        model = (
    ...
    def _detect_bios_mode() -> BIOSMode:
        ...
        if platform.system() == "Windows":
            firmware_type = GigabyteProvider._windows_firmware_type()
    ...
    # Gigabyte publishes no documented WMI, PowerShell, or CLI interface for
    # reading or writing individual BIOS settings (researched: no
    # scriptable settings class or enterprise deployment utility exists in
    # vendor or community documentation, unlike Dell/Lenovo/HP, and unlike
    # ASUS's commercial-only Business System Control Interface). @BIOS is a
    # firmware-flashing GUI tool, not a settings API. ``_apply_setting``
    ...
    # registry via inherited ``DefaultProvider`` behavior. If Gigabyte
    # later documents such an interface, register settings here via
    # ``_register_setting`` following the Lenovo/HP pattern.
    ...
    def boot_order(self) -> List[BootDevice]:
        ...
        vendor-specific interface, since Gigabyte does not publish one.
        """
        if platform.system() == "Linux":
        ...
        for index, number in enumerate(ordered_numbers):
            ...
            devices.append(
                BootDevice(
                    ...
                    device_type=GigabyteProvider._boot_device_type(
    ...
    def firmware_update_supported(self) -> bool:
        """Return whether the Gigabyte @BIOS flashing utility is installed.

        This only reports whether the tool is present; @BIOS is a GUI
        flashing tool with no documented silent/scriptable update mode, so
        this provider does not attempt to invoke or drive it.
        """
        return self._locate_at_bios() is not None
    ...
    def check_for_firmware_updates(self) -> bool:
        ...
        Aquila does not itself download or install vendor firmware, and
        @BIOS exposes no scriptable interface to query update availability
    ...
    def _locate_at_bios(self) -> Optional[Path]:
        """Locate the Gigabyte @BIOS utility, when installed, informationally."""
        candidates = (
            Path(r"C:\Program Files (x86)\GIGABYTE\@BIOS"),
            Path(r"C:\Program Files\GIGABYTE\@BIOS"),
        )
    ...
    def tpm_state(self) -> TPMState:
        ...
        generic Windows interface, not a Gigabyte-specific one -- Gigabyte
        does not publish a firmware TPM setting through any scriptable
        interface.
        """
        if platform.system() != "Windows":
            return TPMState()
    ...
    # Gigabyte does not publish a documented interface for reading or
    # setting a numeric battery charge-limit percentage, so
    # ``battery_charge_limit_supported()`` correctly falls back to
    # ``DefaultProvider``'s honest ``False``.

    # -------------------------------------------------------------------
    # Gigabyte-specific extensions
    # -------------------------------------------------------------------
    ...
    def board_model(self) -> str:
        ...
        Useful for Gigabyte systems, where the board product name (e.g.
        ``Z790 AORUS ELITE AX``) is often more specific than the chassis
        ...
        base_summary.update(
            {
                ...
                "at_bios_found": self._locate_at_bios() is not None,
            }
        )
        return base_summary
    ...
    def validate_configuration(self) -> List[ValidationError]:
        ...
        issues.append(
            ValidationError(
                ...
                message=(
                    "Gigabyte publishes no documented interface for "
                    "individual BIOS setting management; only firmware "
                    "identity, boot order, TPM state, and battery presence "
                    "are available on this provider."
                ),
    ...
    def _query_wmi_safe(self, namespace: str, query: str) -> List[Any]:
        ...
        try:
            ...
            self._gigabyte_logger.debug("WMI query was unavailable: %s", exc)
            return []
    ...
    def _first_property(rows: List[Any], name: str) -> str:
        for row in rows:
            value = GigabyteProvider._safe_property_value(row, name)
            if value:
                return str(value)
